...
process :: [Char] -> Int
process lst = pairEval (map (\x -> mapDig x) lst)
...
pairEval [] = 0
pairEval [a,r] = a * r
pairEval (a1:r1:rest@(_:r2:_)) | r1 < r2 = -(a1 * r1) + pairEval rest
                               | otherwise = (a1 * r1) + pairEval rest

main  = do
			content <- getContents
			let linesOfFile = lines content
			let l1 = [words lineOfFile | lineOfFile <- linesOfFile]
			let l2 = [splitNum ns ps | (ns:ps:_) <- l1]
			let l3 = [sum $ evalSplit lst | lst <- l2]
			let outputLines = [putStr $ show y | y <- l3]
			seqn outputLines
...
evalSplit :: [String] -> [Int]
evalSplit [] = []
evalSplit [s] = [read s::Int]
evalSplit (o:s:ss) | o == "+" = (read s::Int) : evalSplit ss
                   | o == "-" = -(read s::Int) : evalSplit ss
				   | otherwise = (read o::Int) : evalSplit (s:ss)

splitNum :: String -> String -> [String]
splitNum ns ps = reverse (splitNumRec "" (reverse ns) (reverse ps))

splitNumRec :: String -> String -> String -> [String]
splitNumRec buf [] _ = [buf]
splitNumRec buf _ [] = [buf]
splitNumRec buf on@(n:ns) (p:ps)
                       | p == '+' || p == '-' = buf : [p] : splitNumRec "" on ps
                       | otherwise = splitNumRec (n : buf) ns ps
...
process lst = if cnt > hlf then (show maj) else "None"
				where
					maj = findMajCandidate lst 0 0
					cnt = cntCandidate lst maj
					hlf = (length lst) `div` 2
					
findMajCandidate :: [Int] -> Int -> Int -> Int
findMajCandidate [] _ m = m
findMajCandidate (n:ns) c m | n == m = findMajCandidate ns (c + 1) m
							| c == 0 = findMajCandidate ns 1 n
							| otherwise = findMajCandidate ns (c - 1) m

cntCandidate :: [Int] -> Int -> Int
cntCandidate lst m = foldr (\x c -> if x == m then (c + 1) else c) 0 lst
